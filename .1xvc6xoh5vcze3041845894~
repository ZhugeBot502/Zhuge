...
async function onCall({ message, getLang, args }) {
  ...
  if (message.body && ("even" == message.body.toLowerCase() || "odd" == message.body.toLowerCase())) {
    ...
    if (result === message.body.toLowerCase()) {
      // Winner
      const betAmount = bcl.money;
      const prizeAmount = betAmount * 2; // 2 times the bet
      const winnerName = i.name;

      // Increase the money of the winner
      await Users.increaseMoney(i.userID, prizeAmount);

      // Update the message to reflect the winner's prize
      global.api.sendMessage(`${winnerName} chose ${result}. They win ${prizeAmount}$!`, message.threadID, message.messageID);
    } else {
      // Loser
      global.api.sendMessage(`${i.name} chose ${message.body.toLowerCase()}. They lost ${bcl.money}$!`, message.threadID, message.messageID);
    }

    // Mark the player as chosen
    bcl.player.splice(c, 1);
    bcl.player.push({
      name: i.name,
    ...
    if (m != u) return; {
      ...
      global.api.sendMessage({
        ...
        }), global.api.unsendMessage(data.messageID), 2000)
