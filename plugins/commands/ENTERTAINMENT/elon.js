...
export const config = {
  name: "elon",
  version: "1.0.1",
  credits: "SaikiDesu (Converted by Grim)",
  description: "Elon Musk post.",
  usage: "[text]",
  cooldown: 5,
};

export function saikiiWrap(ctx, text, maxWidth) {
...
export async function onCall({ message, args }) {
  const pathImg = path.join(global.cachePath, `${message.threadID}_${Date.now()}_elon.jpg`);
  var text = args.join(" ");
