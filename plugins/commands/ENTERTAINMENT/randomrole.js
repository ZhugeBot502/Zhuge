...
export async function onCall({ message }) {
  ...
  const pathImg = path.join(global.cachePath, `randomrole_${message.threadID}.webp`);
  var callback = () => message.reply({ body: `Random Girl Images Role`, attachment: fs.createReadStream(pathImg)}, () => fs.unlinkSync(pathImg));
  return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(pathImg)).on("close", () => callback());
};