...
export async function onCall({ api, message, args, Users, prefix }) {
  ...
  let pathImg = path.join(
    global.cachePath,
    `${Date.now()}_gfx3_${message.messageID}.png`
  );
  let [text1, text2] = args.join(' ').split('|').map((text) => text.trim());
  if (text1 && text1.length > 6) {
  ...
  if (!text1 || !text2) {
    return global.api.sendMessage(
      `Wrong format\nUse: ${prefix}${config.name} [text] | [text2]`,
      threadID,
      messageID
    );
  }
  ...
  let getWanted = (
    await axios.get(`https://tanjiro-api.onrender.com/gfx3?text=${text1}&text2=${text2}&api_key=tanjiro`, {
