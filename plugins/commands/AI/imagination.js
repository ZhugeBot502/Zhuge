...
export async function onCall({ prefix, args, message }) {
  try {
    ...
    await message.reply({
      body: `🖼️ | Here's your Imagination:`,
      attachment: imageStream,
    });
    message.react("✅");
    await global.api.unsendMessage(waitingMessage.messageID);
  } catch (error) {
    console.log(error);
    message.reply("Failed.").then(() => {
      global.unsendMessage(waitingMessage.ID);
    });
  }
};