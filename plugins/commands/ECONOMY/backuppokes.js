...
const pokemonNames = ["Pikachu", "Charmander", "Marill", "Squirtle", "Bulbasaur", "Meowth", "Vulpix", "Growlithe", "Vaporeon", "Sylveon", "Cyndaquil", "Totodile", "Piplup", "Froakie", "Pidgey", "Elekid", "Abra", "Seel", "Eevee", "Jigglypuff", "Chikorita", "Munchlax", "Wynaut", "Caterpie", "Clefairy", "Psyduck", "Scyther"];
...
async function onCall({ message, getLang, args }) {
  ...
  if (args[0] === "feed") {
    ...
    const maxLevel = 30; // Assuming the maximum level is 30
  ...
  if (args[0] === "battle") {
      ...
      // Deduct the bet amount from the user's balance immediately
      await Users.decreaseMoney(senderID, betAmount);

      // Generate a random result for the battle
      const battleResult = Math.random() < 0.5; // 50% chance of winning
      ...
      if (battleResult) {
          await Users.increaseMoney(senderID, betAmount * 2); // Double the bet amount on win
      }
  ...
  if (args[0] === "accept") {
    ...
    const userBalance = await Users.getMoney(senderID);
    const challengerBalance = await Users.getMoney(challengerID);

    if (betAmount > challengerBalance) {
      return message.reply("You don't have enough credits to accept this bet.");
    }

    // Deduct the bet amount from both users
    const newSenderBalance = await Users.decreaseMoney(senderID, betAmount);
  ...
  if (args[0] === "decline") {
    ...
    // Return the bet amount to the challenger
    const { challengerID, betAmount } = tradeRequests.get(senderID);
