...
async function onCall({ message, getLang, args }) {
  ...
  const chick = (await axios.get("https://i.imgur.com/OXr2KkH.jpg", {
    responseType: "stream"
  })).data;
  ...
  const opponentIDs = Object.keys(mentions);
  ...
  if (args[0] === "bet") {
    ...
    setTimeout(async () => {
      await message.reply(betResultMessage.replace("{opponentName}", "𝚝𝚑𝚎 𝚘𝚙𝚙𝚘𝚗𝚎𝚗𝚝").replace("{amount}", winAmount));
  ...
  if (args[0] === "challenge") {
    ...
    if (opponentIDs.length !== 1) {
      // Check if more than one user is mentioned for the challenge
      return message.reply("𝚈𝚘𝚞 𝚌𝚊𝚗 𝚘𝚗𝚕𝚢 𝚌𝚑𝚊𝚕𝚕𝚎𝚗𝚐𝚎 𝚘𝚗𝚎 𝚞𝚜𝚎𝚛 𝚊𝚝 𝚊 𝚝𝚒𝚖𝚎!");
    }
