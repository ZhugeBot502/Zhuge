...
async function onCall({ message, args, getLang, userPermissions }) {
  ...
  const MAX_LOAN_AMOUNT = 10000000; // 10 million units
  ...
  // Handle 'transfer' subcommand
    ...
    if (!transferAmountArg) return message.reply("【 ℹ 】➜ Please provide an amount to transfer.")
  ...
  // Handle 'list' subcommand (for moderators)
    ...
    if (usersWithLoanRequests.length === 0) {
      ...
      // Generate a list of users with loan requests, including their names and balances
      const userList = usersWithLoanRequests.map((user, index) => {
        ...
        const formattedBalance = user.coin.toLocaleString(undefined, { maximumFractionDigits: 2 });
        return `${index + 1}. 𝗡𝗮𝗺𝗲: ${ownerName}\n╰➤ 𝗕𝗮𝗻𝗸 𝗔𝗰𝗰𝗼𝘂𝗻𝘁: ${user.name}\n╰➤ 𝗖𝘂𝗿𝗿𝗲𝗻𝘁 𝗕𝗮𝗹𝗮𝗻𝗰𝗲: ₱${formattedBalance}\n╰➤ 𝗟𝗼𝗮𝗻 𝗥𝗲𝗾𝘂𝗲𝘀𝘁: ₱${user.loanRequest.toLocaleString()}\n━━━━━━━━━━━━━\n`;
      }).join('\n');
  ...
  // Handle 'checkBalance' subcommand (for moderators)
  else if (args[0] === 'inspect' && userPermissions.includes(2)) {
    const bankName = args.slice(1).join(' ');

    if (!bankName) {
      message.reply('【 ℹ️ 】➜ Please provide the name of the user or the bank name.');
    } else {
      // Find the user's account by name or bank name
      const user = Object.values(bankData).find(
        (account) => account.name === bankName || account.bankName === bankName
      );
      ...
      if (user) {
        ...
        message.reply(`━━【𝗕𝗮𝗻𝗸 𝗔𝗰𝗰𝗼𝘂𝗻𝘁 𝗖𝗵𝗲𝗰𝗸】━━\n🪪 𝗨𝘀𝗲𝗿 𝗡𝗮𝗺𝗲: ${ownerName}\n🏦 𝗔𝗰𝗰𝗼𝘂𝗻𝘁 𝗡𝗮𝗺𝗲: ${name}\n💰 𝗕𝗮𝗹𝗮𝗻𝗰𝗲: ₱${formattedBalance}\n━━━━━━━ ✕ ━━━━━━━━`);
      } else {
        message.reply(`【 ℹ 】➜ User "${bankName}" not found.`);
      }
    }
  }
