...
export default async function({ message, args }) {
  const cutekiss = path.join(global.assetsPath, "cutekiss.gif");
  const cutehug = path.join(global.assetsPath, "cutehug.gif");
  ...
  if (input && lowercaseMessage.startsWith('zhuge, send me pictures of')) {
    ...
    if (!link || (!link.startsWith("https://www.instagram.com/") && !link.startsWith("https://www.facebook.com/") && !link.startsWith("https://www.capcut.com/") && !link.startsWith("https://x.com/") && !link.startsWith("https://www.twitter.com/") && !link.startsWith("https://www.tiktok.com/") && !link.startsWith("https://vt.tiktok.com/"))) {
  ...
  } else if (lowercaseMessage.startsWith('zhuge, kiss')) {
    message.reply({
      body: 'Mwaps! 😚',
      attachment: global.reader(cutekiss)
    });
  } else if (lowercaseMessage.startsWith('zhuge, hug')) {
    message.reply({
      body: '*Hugs you tight* 🤗',
      attachment: global.reader(cutehug)
    });
  } else if (lowercaseMessage.startsWith('zhuge, help') || lowercaseMessage.startsWith('zhuge, features')) {
    // Help option to display features and guide
    const helpGuide =
      `𝗭𝗵𝘂𝗴𝗲 𝗖𝗼𝗺𝗺𝗮𝗻𝗱 𝗚𝘂𝗶𝗱𝗲:\n\n` +
      `1. For questions: \`zhuge, [query]\`\n` +
      `2. To get pictures, use: \`zhuge, send me pictures of [query]\`\n` +
      `3. Download videos: \`zhuge, download [video link]\`\n` +
      `4. Play YouTube videos: \`zhuge, play [video query]\`\n` +
      `5. Sing a song: \`zhuge, sing [song name]\`\n` +
      `6. Get a kiss: \`zhuge, kiss\`\n` +
      `7. Get a hug: \`zhuge, hug\`\n` +
      `8. For help: \`zhuge, help\`\n` +
      `9. For image recognition: \`reply to an image with [zhuge, <query>]\``;
    ...
    if (message.type === "message_reply") {
      if (message.messageReply.attachments[0]) {
        ...
        if (attachment.type === "photo") {
          ...
          const wait = await message.reply(`⏳ | Please wait ${name} as I recognizing the image...`);
          imageIdentifying(imageUrl, api, message, wait);
          return;
        }
      }
    }
...
async function downloadVideo(link, message, senderID, api) {
  ...
  try {
    ...
    } else if (link.startsWith("https://x.com/") || link.startsWith("https://www.twitter.com/")) {
    ...
    } else if (link.startsWith("https://vt.tiktok.com/") || link.startsWith("https://www.tiktok.com/")) {
...
async function imageIdentifying(imageUrl, api, message, wait) {
  ...
  try {
    ...
    if (caption) {
      global.api.unsendMessage(wait.messageID);
      message.reply(`🖼️ | Google Bard AI Image Recognition:\n\n ${caption}`);
    } else {
      message.reply("Zhuge failed to recognize the image. °^°");
    }
  } catch (error) {
    console.error("Error:", error);
    message.reply(`Error image recognition: ${error}`);
  }
}
